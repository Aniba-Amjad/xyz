...
# Load the model name from Hugging Face for summarization (if available)
model_name = "meta-llama/Llama-2-8b-hf"  # Adjust to the LLaMA model from Hugging Face

# Load the tokenizer
tokenizer = AutoTokenizer.from_pretrained(model_name)

# Load the model on CPU without quantization (no GPU, full precision)
...
def get_insight_from_groq(content):
    """Fetches insights from Groq API using the 'llama3-8b-8192' model."""
    chat_completion = client.chat.completions.create(
        messages=[{"role": "user", "content": content}],
        model="llama3-8b-8192",  # Groq model for insights
    )
